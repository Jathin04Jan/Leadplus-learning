...
| D8 | **Duplicate shared rows break lead delete with a 500.** `CampaignMembershipGuard.resolveContactIdSet` calls `leadContactRepository.findByEmailAndTenantIdIsNull(email)`, which returns `Optional` and therefore assumes **at most one** shared (platform) row per email. The live pool has **141 emails with two or more `tenant_id IS NULL` rows**, so deleting a contact whose email is one of them throws `NonUniqueResultException` → `IncorrectResultSizeDataAccessException` → HTTP 500. Reproduced on `aaron.lee@techwave.com` (ids 41982 + 41987, both shared, created 2026-05-18 and 05-19). Introduced by the wave-2 lead-pool remodel (`6f3f4af`), i.e. **already on `main`**, not from the wave-3 port. The same one-shared-row assumption appears in `resolveCompanyIdSet` (keyed by domain). Independently corroborated: intel-search's own corpus analysis found the identical duplicate-shared-row condition (`ARCHITECTURE.md` §5.4 — "58 are duplicate SHARED rows"). | 🔴 | 🔵 | `leadgen/search/service/CampaignMembershipGuard.java:63`. Either dedupe the shared pool, or make the lookup tolerate multiples (prefer the active row, or collect all ids — the method builds a `Set<Long>` anyway, so returning several is harmless). The unique constraint `uq_lead_contact_email_tenant` (`UNIQUE NULLS NOT DISTINCT`) should have prevented these — check whether it is actually applied to this database. |
...
| S9 | **Mailgun system emails interpolate values into HTML with no escaping** — `SystemEmailTemplateRenderer.render` substitutes `{{VAR}}` with raw values, so `MESSAGE` (feedback), `WORKSPACE_NAME`, `COMPANY_NAME` and `USERNAME` are raw-HTML sinks. The SES path escapes via SES Handlebars, so this exposure appeared when MAILGUN became the default provider (PR #58). Limark has the identical behaviour, and every affected value is echoed back to the user who supplied it, so severity is low — but it is **new to this app** and was inherited knowingly rather than accidentally. | 🟡 | 🔵 | `shared/workspace/service/SystemEmailTemplateRenderer.java`. Escape on substitution, or render through a templating engine that escapes by default. Deliberately not fixed in PR #58 — it would change rendered output vs legacy and muddy the port review. |
...
| A8 | **`playwright-extractor` navigation heuristics have no test coverage** — PR #57 changed SPA navigation reliability (direct `careers_url` landing, a same-URL skip, a `domcontentloaded` fallback after a networkidle timeout). `test/` holds only `inputs` and `run.sh`; nothing asserts the described behaviour ("prevent page.evaluate failures"). These are browser-timing heuristics, the hardest category to get right and the easiest to silently regress. Separately, the same-URL skip compares `page.url.rstrip("/")` against `href.rstrip("/")` — any redirect, appended query string, locale path or `www.`/case difference makes it not fire, and redirecting SPA job boards are exactly what it targets, so it may not engage where intended. | 🟠 | 🔵 | `Leadplus-corelabs/playwright-extractor/src/actions/navigation.py`, `server.py`. Add coverage; measure the skip's hit rate against `Docs/Test Companies URL.xlsx` before assuming it works. Note the service is **not currently running** (nothing on :8000, all 3 Java-side scraper schedulers `enabled: false`), so this code has never executed in Corelabs. |
...
| **Scraper** (technographic/job enrichment) | Built, **dormant** — 3 schedulers disabled, empty key. (The dead `limarktech.com` URL is gone: `scraper.base-url` now points at `http://localhost:8000`, the local `playwright-extractor` — which is itself not running.) | P2, A8 |
| **System email** (verification, reset, invites, OTP) | ~~Silently failing~~ **WORKING as of PR #58** — `system-email.provider` selects Mailgun or SES at runtime; Mailgun credentials wired locally and delivery confirmed | G2 |
...
| G2 | **System-email templating never migrated — and the provider switch with it.** Limark's `application/email/SystemEmailTemplate` (enum) + `SystemEmailTemplateRenderer` (`TEMPLATE_DIR = "email-templates/"`) had no Corelabs counterpart, so the **11** `resources/email-templates/*.html` files sat in the repo with **zero code reading them**. Added in Limark commit `986a82e` — the **wave-1** feature commit — so **PRs #41/#42 missed it**. ⚠️ **The original severity assessment below was wrong** (see the correction note under this table). | 🔴 | 🟢 | **FIXED** — PR #58 (`6c98386`). Ported the enum + renderer into `shared/workspace/service` (package-private) and replaced Limark's hardcoded `IS_MAILGUN_ENABLED` boolean with the runtime property `system-email.provider` (`MAILGUN` \| `SES`, default MAILGUN). Verified end-to-end: Mailgun `delivered`. |
| G2b | **Correction to G2's original assessment (2026-08-10).** G2 was recorded as *"Not broken (both paths send mail)"* and severity 🟠. Both were wrong. Corelabs' SES credentials are blank **and** `EmailService.safeSend` swallows the failure, so **every system email had been failing silently since the modular migration** — password reset, email verification, workspace/collaborator invites, OTP, user/vendor approve-reject. This is the real reason local signups never received a verification email and accounts had to be verified with a manual `UPDATE tenant_user SET email_verified = true`. The "workaround" documented in `Docs/RUN-AND-TEST-GUIDE.md` §12 was masking a genuine defect, not a local-environment limitation. Meanwhile Limark/production has been sending these via **Mailgun** all along (`IS_MAILGUN_ENABLED = true`). | 🔴 | 🟢 | Lesson for the register: "both paths exist" is not evidence that either path *works*. Verify a code path end-to-end before recording it as healthy. |
